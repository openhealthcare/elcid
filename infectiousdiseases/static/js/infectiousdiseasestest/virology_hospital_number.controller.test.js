describe('VirologyHospitalNumberCtrl', function(){
    "use strict";
    ...
    beforeEach(function(){
        ...
        mkcontroller = function(with_hosp_num){
            controller = $controller('VirologyHospitalNumberCtrl', {
                $scope:         $scope,
                $timeout:       $timeout,
                $modal:         $modal,
    ...
    describe('addForPatientWithActiveEpisode()', function(){
        ...
        describe('if currently an inpatient and marked as to follow up with the same tag', function(){
          it('should add an episode', function(){
              ...
              activePatientData.episodes[3].category_name = 'Inpatient';
              activePatientData.episodes[3].location[0].category = 'Followup';
              activePatientData.episodes[3].tagging = [{
                virology: true,
                id: 31
              }];
              $scope.tags = {tag: "virology", subrag: ""}
              $scope.newForPatientWithActiveEpisode(activePatientData);
              expect($modal.open).toHaveBeenCalled()
              var callArgs = $modal.open.calls.argsFor(0)[0];
              expect(callArgs.controller).toEqual('ConfirmDischargeCtrl');
              expect(callArgs.templateUrl).toEqual('/templates/modals/confirm_discharge.html');
              expect(callArgs.resolve.nextStepController()).toEqual('AddEpisodeCtrl');
          })
        });
